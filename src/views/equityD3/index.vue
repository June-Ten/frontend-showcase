...
import {
  collapseAllD3EquityNodes,
  createD3EquityGraph,
  expandAllD3EquityNodes,
  resetD3EquityGraph,
} from './createD3EquityGraph'
import { companyInfo, equityGraphData } from '../equity/equityData'

const chartRef = ref<HTMLElement | null>(null)
let graph: ReturnType<typeof createD3EquityGraph> | null = null
...
function initChart() {
  ...
  graph = createD3EquityGraph(chartRef.value, equityGraphData)
}

async function handleReset() {
  if (!graph) return
  await resetD3EquityGraph(graph, equityGraphData)
}

function handleExpandAll() {
  if (!graph) return
  expandAllD3EquityNodes(graph)
}

function handleCollapseAll() {
  if (!graph) return
  collapseAllD3EquityNodes(graph)
}
